...
function AdministracionRusell() {
    ...
    function activarEdicion(prenda) {
        ...
        setDetalle(prenda.detalle || "")
        setStock(prenda.stock !== undefined ? String(prenda.stock) : "0")
                    ...
                    {prendas.map(prenda => (
                        <div key={prenda.id} className="admin-catalog-card">
                            ...
                            <p style={{ color: prenda.stock > 0 ? '#10b981' : '#ef4444', fontWeight: 'bold', fontSize: '0.9rem' }}>
                                Stock: {prenda.stock !== undefined ? prenda.stock : 'N/A'}
                            </p>
                            <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                                ...
                                <button onClick={() => eliminarPrenda(prenda.id)} style={{ flex: 1, background: '#ef4444', color: 'white', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer' }}>
                                    Eliminar
                                </button>
                            </div>
                        </div>
